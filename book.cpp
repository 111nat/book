#define _CRT_SECURE_NO_WARNINGS //дл€ работы с файлом, а также использовани€ функции scanf
#include <stdio.h>
#include <stdlib.h>
...
void zero(int n, phonebook* darr1) {
	char zero[] = "";
	for (int i = 0; i < n; i++) {
...
void add_person(phonebook* darr1, int i) {
	system("cls");
...
int show_persons(int n, phonebook* darr1) {
	int j = 0;
	for (int i = 0; i < n; i++) {
...
void del(int i, phonebook* darr1) {
	char zero[] = "";
	strcpy(darr1[i].numb, zero);
...
void sort(int n, phonebook* darr1) {
	system("cls");
	int method = 0;
...
void copy(phonebook* darr1, int n) {
	system("cls");
	int z = 0;
	FILE* file;
	file = fopen(path, "wt");
...
void reg(phonebook* darr1, int n) {
	int reg = 0;
...
int main() {
	int more_people = 0;//переменна€ дл€ добавлени€ дполнительных людей, если есть .txt файл
	FILE* f;
	f = fopen(path, "rt");
